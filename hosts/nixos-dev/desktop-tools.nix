# Shared desktop + tools layer for nixos-dev-family hosts.
#
# This holds everything that is NOT physical-machine-specific: the desktop
# environment, dev tooling, 1Password, fonts, the full package set, custom
# shell scripts, and the alternate-DE specialisations. Both the physical
# `nixos-dev` host and the `vm-guest` VM import this so they share one source of
# truth for tools and never drift.
#
# Physical-only settings (bootloader, hardware config, hugepages, intel_pstate
# EPP, touchscreen fix, bluetooth, backup-drive mount, nested-virt kvm opts)
# stay in hosts/nixos-dev/configuration.nix and are NOT here.
